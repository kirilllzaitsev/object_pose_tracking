# source: https://github.com/fundamentalvision/Deformable-DETR?tab=readme-ov-file

import copy

import torch
import torchvision
from pose_tracking.losses import dice_loss
from pose_tracking.metrics import accuracy
from pose_tracking.models.matcher import box_cxcywh_to_xyxy, generalized_box_iou
from pose_tracking.utils.misc import get_world_size, is_dist_avail_and_initialized
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, pred_logits, labels, indices, num_boxes, log=True):
    ...
    @torch.no_grad()
    def loss_cardinality(self, pred_logits, labels):
        """Compute the cardinality error, ie the absolute error in the number of predicted non-empty boxes
        This is not really a loss, it is intended for logging purposes only. It doesn't propagate gradients
        """
        device = pred_logits.device
        tgt_lengths = torch.as_tensor([len(v) for v in labels], device=device)
    ...
    def forward(self, outputs, targets):
        ...
        indices = self.matcher(outputs_without_aux, gt_labels=targets["labels"], gt_boxes=targets["boxes"])
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                indices = self.matcher(aux_outputs, gt_labels=targets["labels"], gt_boxes=targets["boxes"])
                for loss in self.losses:
                    if loss == "masks":
        ...
        if "enc_outputs" in outputs:
            ...
            for i, bt in enumerate(bin_targets["labels"]):
                bin_targets["labels"][i] = torch.zeros_like(bt)
            indices = self.matcher(enc_outputs, gt_labels=bin_targets["labels"], gt_boxes=bin_targets["boxes"])
            for loss in self.losses:
                if loss == "masks":
                    # Intermediate masks losses are too costly to compute, we ignore them.
                    continue
