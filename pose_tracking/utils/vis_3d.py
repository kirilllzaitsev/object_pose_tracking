...
def convert_numpy_to_o3d(numpy_points, color=None):
    ...
    if color is not None:
        colors = np.repeat(np.array(color).reshape(-1, 3), numpy_points.shape[0], axis=0)
...
def draw_camera(cam_pose, size=0.1, color=(1, 0, 0), z=1):
    # add camera form
    points = np.array([[0, 0, 0], [-1, -1, z], [-1, 1, z], [1, -1, z], [1, 1, z]]).reshape(-1, 3)
    lines = np.array([[0, 1], [0, 2], [0, 3], [0, 4], [1, 2], [2, 4], [4, 3], [3, 1]]).reshape(-1, 2)
    points = points * size
    ...
    if color is not None:
        if isinstance(color, np.ndarray):
            ...
            new_colors = np.repeat(np.array(color).reshape(-1, 3), lines.shape[0], axis=0)
...
def vis_pcl_cam_o3d(geometries: list[dict]):
    vis = o3d.visualization.Visualizer()
    vis.create_window()
    for geo in geometries:
        if geo["name"] == "points_cloud":
            if "color" in geo:
                pcd = o3d.geometry.PointCloud(convert_numpy_to_o3d(geo["data"], color=geo["color"]))
            else:
                pcd = o3d.geometry.PointCloud(convert_numpy_to_o3d(geo["data"]))
