...
class Backbone(nn.Module):
    ...
    def __init__(
        self, backbone_name: str, train_backbone: bool, return_interm_layers: bool
    ):
        """
        初始化一个 Backbone
        ...
        # ), f"Backbone do not support '{backbone_name}'."
        # backbone = resnet50(
    ...
    def forward(self, ntensor: NestedTensor):
        ...
        for name, output in outputs.items():
            ...
            masks = F.interpolate(
                masks[None].float(), mode="nearest", size=output.shape[-2:]
            ).to(masks.dtype)[0]
...
class BackboneWithPE(nn.Module):
    ...
    def forward(
        self, ntensor: NestedTensor
    ) -> (List[NestedTensor], List[torch.Tensor]):
        ...
        return (
            features,
            pos_embeds,
        )  # (B, C, H, W), (B, 2*num_pos_feats, H, W)，C is different in different layers.
...
def build(config: dict) -> BackboneWithPE:
    ...
    backbone = Backbone(
        backbone_name=config["BACKBONE"], train_backbone=True, return_interm_layers=True
    )
