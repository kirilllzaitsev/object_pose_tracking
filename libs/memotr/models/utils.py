...
def query_masks_to_attn_mask(query_mask: torch.Tensor, n_heads: int, src_len: int):
    attn_mask = torch.ones(
        (query_mask.shape[0], 1, query_mask.shape[1], query_mask.shape[1]),
        dtype=torch.bool,
    ...
    attn_mask = attn_mask.reshape(
        query_mask.shape[0] * n_heads, query_mask.shape[1], query_mask.shape[1]
    )
    return attn_mask
...
def load_pretrained_model(
    model: nn.Module, pretrained_path: str, show_details: bool = False
):
    ...
    for k in pretrained_keys:
        if k in model_state_dict:
            ...
            if (
                ...
                == model_state_dict["det_query_embed"].shape
            ):
                pretrained_state_dict["det_query_embed"] = pretrained_state_dict[
                    k
                ].clone()
            else:
                print(f"Det Query shape is not equal. Check if you turn on 'USE_DAB'.")
                pretrained_state_dict["det_query_embed"] = model_state_dict[
                    "det_query_embed"
                ]
            ...
            if pretrained_state_dict[k].shape == model_state_dict["det_anchor"].shape:
                ...
                print(
                    f"Pretrain model's query num is {pretrained_state_dict[k].shape[0]}, "
    ...
    # 80 missing from dab-def-detr by default
