...
from memotr.utils.box_ops import box_cxcywh_to_xyxy, box_iou_union
from memotr.utils.utils import inverse_sigmoid
from torch.utils.checkpoint import checkpoint

from .ffn import FFN
from .mlp import MLP
from .utils import logits_to_scores, pos_to_pos_embed
...
class QueryUpdater(nn.Module):
    ...
    def forward(
        ...
        tracks = self.select_active_tracks(
            previous_tracks, new_tracks, unmatched_dets, no_augment=no_augment
        )
    ...
    def update_tracks_embedding(self, tracks: List[TrackInstances]):
        for b in range(len(tracks)):
            ...
            if self.visualize:
                ...
                torch.save(
                    tracks[b].ref_pts.cpu(),
                    "./outputs/visualize_tmp/query_updater/current_ref_pts.tensor",
                )
                ...
                torch.save(
                    tracks[b].query_embed.cpu(),
                    "./outputs/visualize_tmp/query_updater/current_query_feat.tensor",
                )
                torch.save(
                    tracks[b].ids.cpu(),
                    "./outputs/visualize_tmp/query_updater/current_ids.tensor",
                )
                torch.save(
                    tracks[b].labels.cpu(),
                    "./outputs/visualize_tmp/query_updater/current_labels.tensor",
                )
                torch.save(
                    scores.cpu(),
                    "./outputs/visualize_tmp/query_updater/current_scores.tensor",
                )
            if self.use_dab:
            ...
            query_pos = pos_to_pos_embed(
                tracks[b].ref_pts.sigmoid(), num_pos_feats=self.hidden_dim // 2
            )
            ...
            # Update Long Memory
            long_memory = (
                1 - self.long_memory_lambda
            ) * long_memory + self.long_memory_lambda * tracks[b].output_embed
            tracks[b].long_memory = tracks[b].long_memory * ~is_pos.reshape(
                (is_pos.shape[0], 1)
            ) + long_memory * is_pos.reshape((is_pos.shape[0], 1))
            # Update Last Outputs Embedding
            tracks[b].last_output = tracks[b].last_output * ~is_pos.reshape(
                (is_pos.shape[0], 1)
            ...
            if self.use_dab:
                ...
                new_query_pos = self.linear_pos2(
                    self.activation(self.linear_pos1(output_embed))
                )
            ...
            if self.visualize:
                torch.save(
                    tracks[b].ref_pts.cpu(),
                    "./outputs/visualize_tmp/query_updater/next_ref_pts.tensor",
                )
                ...
                torch.save(
                    tracks[b].ids.cpu(),
                    "./outputs/visualize_tmp/query_updater/next_ids.tensor",
                )
                torch.save(
                    tracks[b].labels.cpu(),
                    "./outputs/visualize_tmp/query_updater/next_labels.tensor",
                )
                torch.save(
                    scores.cpu(),
                    "./outputs/visualize_tmp/query_updater/next_scores.tensor",
                )
        ...
        if self.training:
            for b in range(len(new_tracks)):
                ...
                if self.tp_drop_ratio == 0.0 and self.fp_insert_ratio == 0.0:
                    active_tracks = TrackInstances.cat_tracked_instances(
                        previous_tracks[b], new_tracks[b]
                    )
                    active_tracks = TrackInstances.cat_tracked_instances(
                        active_tracks, unmatched_dets[b]
                    )
                    scores = torch.max(
                        logits_to_scores(logits=active_tracks.logits), dim=1
                    ).values
                    keep_idxes = (scores > self.update_threshold) | (
                        active_tracks.ids >= 0
                    )
                    ...
                    active_tracks = active_tracks[
                        (active_tracks.iou > 0.5) & (active_tracks.ids >= 0)
                    ]
                    if self.tp_drop_ratio > 0.0 and not no_augment:
                        if len(active_tracks) > 0:
                            tp_keep_idx = (
                                torch.rand((len(active_tracks),)) > self.tp_drop_ratio
                            )
                    ...
                    if self.fp_insert_ratio > 0.0 and not no_augment:
                        ...
                        if (
                            len(unmatched_dets[b]) > 0
                            and len(selected_active_tracks) > 0
                        ):
                            fp_num = len(selected_active_tracks)
                            if fp_num >= len(unmatched_dets[b]):
                                insert_fp = unmatched_dets[b]
                            else:
                                selected_active_boxes = box_cxcywh_to_xyxy(
                                    selected_active_tracks.boxes
                                )
                                unmatched_boxes = box_cxcywh_to_xyxy(
                                    unmatched_dets[b].boxes
                                )
                                iou, _ = box_iou_union(
                                    unmatched_boxes, selected_active_boxes
                                )
                                fp_idx = torch.max(iou, dim=0).indices
                                fp_idx = torch.unique(fp_idx)
                                insert_fp = unmatched_dets[b][fp_idx]
                            active_tracks = TrackInstances.cat_tracked_instances(
                                active_tracks, insert_fp
                            )

                if len(active_tracks) == 0:
                    device = next(self.query_feat_ffn.parameters()).device
                    fake_tracks = TrackInstances(
                        frame_height=1.0, frame_width=1.0, hidden_dim=self.hidden_dim
                    ).to(device=device)
                    if self.use_dab:
                        fake_tracks.query_embed = torch.randn(
                            (1, self.hidden_dim), dtype=torch.float, device=device
                        )
                    else:
                        fake_tracks.query_embed = torch.randn(
                            (1, 2 * self.hidden_dim), dtype=torch.float, device=device
                        )
                    fake_tracks.output_embed = torch.randn(
                        (1, self.hidden_dim), dtype=torch.float, device=device
                    )
                    if self.use_dab:
                        fake_tracks.ref_pts = torch.randn(
                            (1, 4), dtype=torch.float, device=device
                        )
                    else:
                    ...
                    fake_tracks.ids = torch.as_tensor(
                        [-2], dtype=torch.long, device=device
                    )
                    fake_tracks.matched_idx = torch.as_tensor(
                        [-2], dtype=torch.long, device=device
                    )
                    fake_tracks.boxes = torch.randn(
                        (1, 4), dtype=torch.float, device=device
                    )
                    fake_tracks.logits = torch.randn(
                        (1, active_tracks.logits.shape[1]),
                        dtype=torch.float,
                        device=device,
                    )
                    fake_tracks.iou = torch.zeros(
                        (1,), dtype=torch.float, device=device
                    )
                    fake_tracks.last_output = torch.randn(
                        (1, self.hidden_dim), dtype=torch.float, device=device
                    )
            ...
            active_tracks = TrackInstances.cat_tracked_instances(
                previous_tracks[0], new_tracks[0]
            )
