...
class DeformableDecoder(nn.Module):
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.visualize:
                os.system(
                    f"mv ./outputs/visualize_tmp/decoder/attn_score.tensor "
                    f"./outputs/visualize_tmp/decoder/attn_score_layer_{lid}.tensor"
                )
                os.system(
                    f"mv ./outputs/visualize_tmp/decoder/sampling_locations.tensor "
                    ...
                    if self.use_dab:
                        reference_points = torch.cat(
                            (
                                ...
                                reference_points[:, self.n_det_queries :, :],
                            ),
                            dim=1,
                        )
                    else:  # D-DETR
                        reference_points = torch.cat(
                            (
                                ...
                                ref_pts_backup[:, self.n_det_queries :, :],
                            ),
                            dim=1,
                        )
                else:
...
class DeformableDecoderLayer(nn.Module):
    ...
    def forward_self_attn(self, tgt, query_pos, query_mask):
        ...
        if self.visualize:
            torch.save(
                _[0, :, :].cpu(), "./outputs/visualize_tmp/decoder/attn_score.tensor"
            )
    ...
    def forward_track_attn(self, tgt, query_pos, query_mask):
        ...
        if q.shape[1] > self.n_det_queries:
            tgt2, _ = self.track_attn(
                q[:, self.n_det_queries :],
                k[:, self.n_det_queries :],
                tgt[:, self.n_det_queries :],
    ...
    def forward(
        ...
        if self.extra_track_attn:
            tgt = self.forward_track_attn(
                tgt=tgt, query_pos=query_pos, query_mask=query_mask
            )
        tgt = self.forward_self_attn(
            tgt=tgt, query_pos=query_pos, query_mask=query_mask
        )
        # Cross Attention
