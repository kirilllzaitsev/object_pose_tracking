...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, ntensor: NestedTensor) -> torch.Tensor:
        ...
        dim_i = torch.arange(
            self.num_pos_feats, dtype=torch.float32, device=tensors.device
        )
        dim_i = self.temperature ** (
            2 * (torch.div(dim_i, 2, rounding_mode="trunc")) / self.num_pos_feats
        )
        ...
        pos_embed = torch.cat((pos_y, pos_x), dim=3).permute(
            0, 3, 1, 2
...
def build(config: dict):
    assert (
        config["HIDDEN_DIM"] % 2 == 0
    ), f"Hidden dim should be 2x, but get {config['HIDDEN_DIM']}."
    num_pos_feats = config["HIDDEN_DIM"] / 2
    return PositionEmbeddingSine(
        num_pos_feats=num_pos_feats, normalize=True, scale=2 * math.pi, temperature=20
    )
