    ...
    def forward(self, frame: NestedTensor, tracks: list[TrackInstances]):
        ...
        for level in range(outputs.shape[0]):
            ...
            if self.use_roi:
                rgb = frame.tensors
                hw = rgb.shape[-2:]
                pred_boxes_xyxy = box_cxcywh_to_xyxy(output_bbox)
                rgb_crop = get_crops(rgb, pred_boxes_xyxy, hw=hw, crop_size=(60 * 2, 80 * 2), padding=5)
                crop_feats = self.roi_cnn(rgb_crop)
                crop_feats = rearrange(crop_feats, "(b q) d -> b q d", b=rgb.shape[0], q=output_bbox.shape[1])
                # TODO: check allocentric
                rot_mlp_in = torch.cat([outputs[level], crop_feats], dim=-1)
            else:
                rot_mlp_in = outputs[level]
            outputs_rot = self.rot_embed[level](rot_mlp_in)
            if self.do_predict_2d_t:
